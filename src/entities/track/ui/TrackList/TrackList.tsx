...
export const TrackList: FC<TrackListProps> = ({
  ...
  const columnsToShow = useMemo(
    () => allColumnsDefinitions.filter((columnDef) => columns.includes(columnDef.key)),
    [columns],
  )
  ...
  const likes = useQuery(
    spotifyApi.checkUsersSavedTracks.queryOptions({
      idList: items?.map((item) => item.id),
      override: {
        enabled: isLikesColumn && Boolean(items),
      },
  ...
  // const like = useLike()
  const ref = useOnVisible(onScrollDown)

  return (
    <div className="relative">
      <Table hideHeader={hideHeader} classNames={classNames}>
        ...
        <TableBody isLoading={initialLoading} loadingContent={<Spinner />}>
      ...
      {onScrollDown && (
        <div ref={ref} className="absolute bottom-72">
          SCROLLED
        </div>
      )}
    </div>
